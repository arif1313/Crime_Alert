  ...
  return (
    <div className="flex items-center gap-4 px-5 sm:px-8 py-4 border-b border-white/5 bg-[#0b0b12] sticky top-0 z-30">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select value={area} onChange={(event) => setArea(event.target.value)} aria-label="Filter by area" className="max-w-[150px] rounded-xl border border-white/5 bg-[#14141f] px-3 py-2.5 text-xs text-gray-300 outline-none focus:border-red-500/50"><option value="all">All areas</option>{areas.filter((item) => item !== "all").map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <select value={category} onChange={(event) => setCategory(event.target.value)} aria-label="Filter by category" className="max-w-[150px] rounded-xl border border-white/5 bg-[#14141f] px-3 py-2.5 text-xs capitalize text-gray-300 outline-none focus:border-red-500/50"><option value="all">All categories</option>{["murder", "robbery", "fraud", "assault", "theft", "vandalism", "arson", "other"].map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <select value={status} onChange={(event) => setStatus(event.target.value)} aria-label="Filter by status" className="max-w-[140px] rounded-xl border border-white/5 bg-[#14141f] px-3 py-2.5 text-xs capitalize text-gray-300 outline-none focus:border-red-500/50"><option value="all">All statuses</option><option value="pending">Pending</option><option value="reviewing">Reviewing</option><option value="resolved">Resolved</option></select>
      </div>
      {hasFilters && <div className="flex w-full flex-wrap gap-1.5">
